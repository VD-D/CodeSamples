...
/* Class header. */
#include "Animation/TriggerSkill.h"
...
void UTriggerSkill::Notify(USkeletalMeshComponent* MeshComp, UAnimSequenceBase* Animation)
{
    if (!IsValid(MeshComp))
    {
    ...
    if (AAutobattlerCharacter* OwnerCharacter = Cast<AAutobattlerCharacter>(MeshComp->GetOwner()))
    {
        ...
        {
            AIController->ExecuteSkill(AbilityTypeTriggered);
        }
